...
class Idle:
    ...
    def enter(ch, e):
        ...
        ch.run_state = Idle
        if def_down(e):
            if get_time() - ch.wait_time > def_cool_time:  # ch.time() > 5
                ch.auto_guard = False
    ...
    def exit(ch, e):
        if atk_down(e):
            ch.shoot_ball()
        if skill_down(e):
            if ch.time - Blueteam.skill_wait_time > skill_cool_time:
                ch.use_skill()
        if ch.run == 2:
            ch.angle -= 45
        elif ch.run == 3:
...
class RunRight:
    ...
    def enter(ch, e):
        ...
        if def_down(e):
            if get_time() - ch.wait_time > def_cool_time:  # ch.time() >
                ch.auto_guard = False
                ch.state_machine.handle_event(('LETS_DEFENSE', 0))
        if a_down(e):
            ch.angle += radian
...
class Blueteam:
    ...
    def use_skill(self):
        if self.getball == True:
            ...
            Blueteam.skill_wait_time = self.time
    ...
    def draw(self):
        ...
        if float(self.auto_wait_time) + 15 - self.time > 0:
            self.font.draw(self.x - 10, self.y + 80, f'{float(self.auto_wait_time) + 15 - self.time:.1f}',
                           (255, 255, 0))
